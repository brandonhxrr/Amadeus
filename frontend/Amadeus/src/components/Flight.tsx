import { ArrowRightIcon, ArrowsRightLeftIcon, ChevronDownIcon, ChevronUpIcon } from "@heroicons/react/16/solid";
import { FlightSegment } from "./FlightSegment";
import { PricePerTraveler } from "./PricePerTraveler";
import { Dictionaries, FareDetailsBySegment, Fee, Itinerary, Price, TravelerPricing } from "./SearchResponse";
...
const sumFees = (fees: Fee[]): number => {
    let sum: number = 0;

    fees.forEach((fee) => {
        sum += parseInt(fee.amount, 10);
    });

    return sum;
}

const Flight: React.FC<FlightProps> = ({ itineraries, dictionaries, price, travelerPricings }) => {
        ...
        <div>
            <div className="w-full p-4 mt-10 bg-white border border-gray-200 rounded-t-lg rounded-b-lg shadow sm:p-8 sm:px-20 dark:bg-gray-800 dark:border-gray-700">
                <div className="flex items-center mb-5">
                    ...
                    <ArrowsRightLeftIcon className="w-5 h-5 me-3 text-blue-600"/>
                    :
                    <ArrowRightIcon className="w-5 h-5 me-3 text-blue-600"/>
                    }
                    <p className=" sm:text-lg dark:text-gray-400 text-left text-4xl font-bold">{itineraries[0].segments[0].departure.iataCode} - {itineraries[0].segments[0].arrival.iataCode}</p>
                </div>
                <div className="flex w-full">
                    <div className="flex w-full items-center h-56 text-center divide-gray-700">
                        <div className="w-1/5 h-full flex flex-col justify-center">
                            ...
                            <p className="font-mono font-bold text-3xl mt-7 mb-2">{getParsedTime(itineraries[0].segments[0].departure.at as string)}</p>
                            <p className="font-mono font-bold text-sm my-1">{getParsedDate(itineraries[0].segments[0].departure.at as string)}</p>
                            <p>Airport: {itineraries[0].segments[0].departure.iataCode}</p>
                        </div>
                        ...
                        <div className="w-1/5 h-full flex flex-col justify-center">
                            <p className="font-mono font-bold uppercase">{dictionaries.carriers[itineraries[0].segments[0].carrierCode]}</p>
                            ...
                            <p className="font-mono font-bold uppercase rounded-full bg-gradient-to-r from-blue-600 to-sky-300 text-white w-fit mx-auto px-5">{parseISODurationToTime(itineraries[0].duration as string)}</p>
                        </div>
                        ...
                        <div className="w-1/5 h-full flex flex-col justify-center">
                            ...
                            <p className="font-mono font-bold text-3xl mt-7 mb-2">{getParsedTime(itineraries[0].segments[0].arrival.at as string)}</p>
                            <p className="font-mono font-bold text-sm my-1">{getParsedDate(itineraries[0].segments[0].arrival.at as string)}</p>
                            <p>Airport: {itineraries[0].segments[0].arrival.iataCode}</p>
                        </div>
                    ...
                    <div className="ms-5 border-dotted border-l-4 border-gray-400 content-center">
                        ...
                        <p className="text-xl text-green-600 font-mono font-bold w-full text-right mx-5">${price.total} {price.currency}</p>
                        <p className="text-md text-gray-800 font-mono font-bold w-full text-right mx-5">total</p>
                        <p className="text-xl text-green-600 font-mono font-bold w-full text-right mx-5 mt-3">${travelerPricings[0].price.total} {price.currency}</p>
                        <p className="text-md text-gray-800 font-mono font-bold w-full text-right mx-5">per traveler</p>

                    </div>
                </div>

                <div className="flex justify-center items-center mt-5 cursor-pointer" onClick={() => (setIsFlightDetailsVisible(!isFlightDetailsVisible))}>
                    <p className="font-mono uppercase text-sm font-bold">More details</p>
                    {isFlightDetailsVisible ?
                        <ChevronUpIcon className="w-5 h-5 ms-1" />
                        :
                        ...
                        <div className="flex w-full">
                            <div className="flex-col w-full items-center text-center">
                                {itineraries.map((itinerary) => (
                                    itinerary.segments.map((segment) => (
                                        <FlightSegment fareDetailsBySegment={travelerPricings[0].fareDetailsBySegment.find((s) => s.segmentId === segment.id) as FareDetailsBySegment} segment={segment} dictionaries={dictionaries as Dictionaries} />
                                    ))
                                ))}
                            ...
                            <div className="ms-5 border-dotted border-l-4 border-gray-400 content-center font-mono font-bold uppercase">
                                ...
                                <div className="w-full text-right mx-5">
                                    <p className="text-xl ">Base: <span className="text-green-600">${price.base} {price.currency}</span></p>
                                    <p className="text-xl border-b-2 mb-4">Fees: <span className="text-green-600">${sumFees(price.fees as Fee[])} {price.currency}</span></p>
                                    <p className="text-xl ">Total: <span className="text-green-600">${price.total} {price.currency}</span></p>
                                </div>

                                <p className="text-xl text-right mt-10 m-5 w-full" mx-5>Per traveler</p>

                                {travelerPricings.map((travelerPricing) => (
                                    <PricePerTraveler travelerPricing={travelerPricing} />
                                ))}
