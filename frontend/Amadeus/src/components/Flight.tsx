...
import { FareDetailsBySegment, Fee, Itinerary, Price, TravelerPricing } from "./SearchResponse";
...
const Flight: React.FC<FlightProps> = ({ itinerary, price, travelerPricings }) => {
    return (
        <div>
            <div className="w-full p-4 mt-10 bg-white border border-gray-200 rounded-t-lg rounded-b-lg shadow sm:p-8 sm:px-20 dark:bg-gray-800 dark:border-gray-700">
                <p className="mb-5 sm:text-lg dark:text-gray-400 text-left text-4xl font-bold">{itinerary?.segments[0].departure.iataCode} - {itinerary?.segments[0].arrival.iataCode}</p>
                <div className="flex w-full">
                    <div className="flex w-full items-center h-56 text-center divide-gray-700">
                        <div className="w-1/5 h-full flex flex-col justify-center">
                            ...
                            <p className="font-mono font-bold text-3xl mt-7 mb-2">{getParsedTime(itinerary?.segments[0].departure.at as string)}</p>
                            <p className="font-mono font-bold text-sm my-1">{getParsedDate(itinerary?.segments[0].departure.at as string)}</p>
                            <p>Airport: {itinerary?.segments[0].departure.iataCode}</p>
                        </div>
                        ...
                        <div className="w-1/5 h-full flex flex-col justify-center">
                            <p className="font-mono font-bold uppercase">{itinerary?.segments[0].carrierCode}</p>
                            ...
                            <p className="font-mono font-bold uppercase rounded-full bg-gradient-to-r from-blue-600 to-sky-300 text-white w-fit mx-auto px-5">{ parseISODurationToTime(itinerary?.duration as string) }</p>
                        </div>
                        ...
                        <div className="w-1/5 h-full flex flex-col justify-center">
                            ...
                            <p className="font-mono font-bold text-3xl mt-7 mb-2">{getParsedTime(itinerary?.segments[0].arrival.at as string)}</p>
                            <p className="font-mono font-bold text-sm my-1">{getParsedDate(itinerary?.segments[0].arrival.at as string)}</p>
                            <p>Airport: {itinerary?.segments[0].arrival.iataCode}</p>
                        </div>
                ...
                <div className="flex w-full">
                    <div className="flex-col w-full items-center text-center">
                        {itinerary?.segments.map((segment) => (
                            <FlightSegment fareDetailsBySegment={travelerPricings[0].fareDetailsBySegment.find((s) => s.segmentId === segment.id) as FareDetailsBySegment} segment={segment}  />
                        ))}
